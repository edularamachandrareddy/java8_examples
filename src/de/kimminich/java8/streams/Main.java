...
l.add("cahdnra");
l.add("hai");
l.stream().filter(s -> s.length()>3).collect(Collectors.toList()).forEach(System.out::println);
Arrays.asList(1,2,13,4,15,6,17,8,19).stream().filter(s -> s.intValue()>6).collect(Collectors.toList()).forEach(System.out::println);
